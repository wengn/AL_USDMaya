...
MStatus DirectionalLight::initialize()
{
    ...
    const char * const errorString = "DirectionalLightTranslator: error initializing attributes";
...
bool DirectionalLight::updateUsdPrim(const UsdStageRefPtr& stage, const SdfPath& usdPath, const MObject& mayaObj)
{
    ...
    float pointWorld[3] = {1.0,1.0,1.0};
    MAngle angle;
    float intensity = 0.0;
    float exposure = 0.0;
    float diffuse = 1.0;
    ...
    if(!(pointWorld[0] == 1.0 && pointWorld[1]== 1.0 && pointWorld[2]==1.0))
    {
